...
const QuickAccessManager: React.FC<QuickAccessManagerProps> = ({
  ...
  const filteredAvailable = availableProfessions.filter(p =>
      ...
      <div className="bg-white rounded-lg shadow-xl w-full max-w-4xl mx-4 max-h-[90vh] flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto p-6">
          ...
          <div className="grid grid-cols-2 gap-6">
            {/* Selected Professions (Left Side) */}
            <div>
              ...
              <div className="space-y-2 min-h-[400px] border-2 border-dashed border-gray-300 rounded-lg p-4">
                {selectedProfessions.length === 0 ? (
                  <div className="flex flex-col items-center justify-center h-full text-gray-400 py-20">
                    <Star className="w-12 h-12 mb-2" />
                    <p className="text-sm">No professions selected</p>
                    <p className="text-xs">Add professions from the right panel</p>
                  </div>
                ) : (
                  selectedProfessions.map((profession, index) => (
                    <div
                      ...
                      className={`flex items-center gap-3 p-3 bg-white border-2 rounded-lg transition-all cursor-move ${
                        draggedIndex === index
            ...
            {/* Available Professions (Right Side) */}
            <div>
              ...
              <div className="space-y-2 max-h-[400px] overflow-y-auto border border-gray-200 rounded-lg p-4">
                {filteredAvailable.length === 0 ? (
                  <div className="text-center py-10 text-gray-400">
                    <p className="text-sm">No professions available</p>
                  </div>
                ) : (
                  filteredAvailable.map((profession) => (
                    <button
                      key={profession._id}
                      onClick={() => addProfession(profession)}
                      disabled={selectedProfessions.length >= 8}
                      className="w-full flex items-center justify-between p-3 bg-gray-50 hover:bg-gray-100 rounded-lg transition-colors text-left disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                      <div>
                        <p className="text-sm font-medium text-gray-900">
                          {profession.name}
